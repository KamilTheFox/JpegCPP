...
JpegEncodedData PipelineHuffmanEncoder::encode(const vector<QuantizedBlock>& blocks, 
    ...
    // ???????? ??? ????? (Ð¿Ð¾ÑÐ»ÐµÐ´Ð¾Ð²Ð°Ñ‚ÐµÐ»ÑŒÐ½Ð¾, Ñ‚Ð°Ðº ÐºÐ°Ðº BitWriter Ð½Ðµ thread-safe)
    BitWriter writer;
...
ProcessingPipeline::~ProcessingPipeline() {
    // Ð£ÑÑ‚Ð°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ Ñ„Ð»Ð°Ð³Ð¸ Ð·Ð°Ð²ÐµÑ€ÑˆÐµÐ½Ð¸Ñ
    dctFinished = true;
    quantizationFinished = true;
    
    // Ð‘ÑƒÐ´Ð¸Ð¼ Ð²ÑÐµ Ñ€Ð°Ð±Ð¾Ñ‡Ð¸Ðµ Ð¿Ð¾Ñ‚Ð¾ÐºÐ¸
    dctCV.notify_all();
...
vector<QuantizedBlock> ProcessingPipeline::processImage(const YCbCrImage& image) {
    ...
    // Ð—Ð°Ð¿ÑƒÑÐºÐ°ÐµÐ¼ DCT Ð¸ ÐºÐ²Ð°Ð½Ñ‚Ð¸Ð·Ð°Ñ†Ð¸ÑŽ Ð¿Ð¾Ñ‚Ð¾ÐºÐ¸
    ...
    // Ð˜Ð·Ð²Ð»ÐµÐºÐ°ÐµÐ¼ Ð±Ð»Ð¾ÐºÐ¸ Ð¿Ð°Ñ€Ð°Ð»Ð»ÐµÐ»ÑŒÐ½Ð¾ Ð¸ ÑÑ€Ð°Ð·Ñƒ ÐºÐ»Ð°Ð´ÐµÐ¼ Ð² DCT Ð¾Ñ‡ÐµÑ€ÐµÐ´ÑŒ
    vector<future<void>> extractionFutures;
    
    // Y ÐºÐ¾Ð¼Ð¿Ð¾Ð½ÐµÐ½Ñ‚ - Ð¿Ð¾Ð»Ð½Ð¾Ðµ Ñ€Ð°Ð·Ñ€ÐµÑˆÐµÐ½Ð¸Ðµ
    extractionFutures.push_back(async(launch::async, [&]() {
        for (int by = 0; by < height; by += 8) {
            for (int bx = 0; bx < width; bx += 8) {
                // Ð˜Ð·Ð²Ð»ÐµÐºÐ°ÐµÐ¼ Ð±Ð»Ð¾Ðº
                ...
                // Ð¡Ñ€Ð°Ð·Ñƒ Ð¿Ñ€Ð¸Ð¼ÐµÐ½ÑÐµÐ¼ DCT Ð¸ ÐºÐ»Ð°Ð´ÐµÐ¼ Ð² DCT Ð¾Ñ‡ÐµÑ€ÐµÐ´ÑŒ
                DctBlock dctBlock;
                dctBlock.x = bx;
                dctBlock.y = by;
    ...
    // Cb Ð¸ Cr ÐºÐ¾Ð¼Ð¿Ð¾Ð½ÐµÐ½Ñ‚Ñ‹ - ÑÑƒÐ±ÑÑÐ¼Ð¿Ð»Ð¸Ð½Ð³ 2x2
    extractionFutures.push_back(async(launch::async, [&]() {
    ...
    // Ð–Ð´ÐµÐ¼ Ð·Ð°Ð²ÐµÑ€ÑˆÐµÐ½Ð¸Ñ Ð¸Ð·Ð²Ð»ÐµÑ‡ÐµÐ½Ð¸Ñ
    for (auto& future : extractionFutures) {
        future.get();
    }
    
    // Ð¡Ð¸Ð³Ð½Ð°Ð»Ð¸Ð·Ð¸Ñ€ÑƒÐµÐ¼ Ð·Ð°Ð²ÐµÑ€ÑˆÐµÐ½Ð¸Ðµ DCT ÑÑ‚Ð°Ð´Ð¸Ð¸
    dctFinished = true;
    dctCV.notify_all();
